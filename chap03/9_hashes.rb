...
def shared(a, b)
  hash = Hash.new
  a.each { | item | hash[item] = Array.new }
  b.each { | item | hash[item] = Array.new }
# keys now initialized to empty arrays. Now check for keys in a
  ...
  finalArray << hash
  finalArray << commonNumbers
  finalArray

end

print shared [1,2,3], [1,2,4]
